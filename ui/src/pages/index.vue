<template>
  <v-container>
    <v-row class="border rounded-lg pa-10" v-if="false">
      <v-col cols="6">
        <v-select
          :items="linesArray"
        ...
        <v-slide-y-transition>
          <v-row v-if="selected_line != ''">
            <v-col
              ...
              md="4"
              lg="3"
              v-for="item in allLines[selected_line]['outputs_count']"
            >
              <v-text-field
                :label="'output-' + item"
                class="border-0 mb-n2"
                :color="
                  output_percents[item - 1] < 0 ||
                  output_percents[item - 1] > 100
                    ? 'error'
                    : 'primary'
                "
                density="compact"
                v-model="output_percents[item - 1]"
              />
              <v-slider
                v-model="output_percents[item - 1]"
                step="5"
      ...
      <v-col cols="12" class="pa-1">
        ...
        <ul class="pa-5">
          <li
            class="d-inline-block me-10"
            :class="{
              'text-error': !selected_line,
              'text-success': selected_line,
            }"
          >
          ...
          <li
            class="d-inline-block me-10"
            :class="{
              'text-error': !selected_week,
              'text-success': selected_week,
            }"
          >
          ...
          <li
            class="d-inline-block me-10"
            :class="{ 'text-error': !selected_tc, 'text-success': selected_tc }"
          >
          ...
          <li
            class="d-inline-block me-10"
            :class="{
              'text-error': percents_sum != 100,
              'text-success': percents_sum == 100,
            }"
          >
            <strong>Check Sum: {{ percents_sum }}</strong>
          </li>
        </ul>
      </v-col>
      <v-col cols="12" class="pa-1 mt-5">
        <v-slide-y-transition leave-absolute hide-on-leave>
          <div v-if="!storeValidation" class="mb-5">
            <v-alert type="error" variant="tonal" border
              >There is some problem ...</v-alert
            >
          </div>
        </v-slide-y-transition>
        ...
        <v-row class="text-center">
          ...
          <v-col cols="6">
            <v-btn
              variant="tonal"
              class="border border-success border-opacity-100"
              :loading="loading"
              color="success"
        ...
        <v-slide-y-transition leave-absolute hide-on-leave>
          <v-row v-if="report?.out_number">
            <v-col cols="12"> Lines report: </v-col>
            <v-col cols="12" class="px-10">
              Out Total : {{ report?.out_number["out_total"] }}
              <template v-for="i in 10">
                {{ report.out_number["out-4"] }}
                <div v-if="report?.out_number['out-' + i]">
                  Out {{ i }} : {{ report?.out_number["out-" + i] }}
                </div>
              </template>
            </v-col>
          </v-row>
